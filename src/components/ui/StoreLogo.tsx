...
export function StoreLogo({
  ...
  const sizes = {
    sm: { wrap: "gap-1", icon: "h-5 w-5", title: "text-2xl", sub: "text-[10px]" },
    md: { wrap: "gap-1.5", icon: "h-6 w-6", title: "text-[2rem] md:text-[2.25rem]", sub: "text-xs" },
  ...
  if (!STORE.useTextLogo) {
    return (
      <Link to="/" className={cn("navbar-brand block", className)} aria-label={STORE.name}>
        <img src={CDN.logo} alt={STORE.name} className="logo-normal h-12 w-auto object-contain" height={48} />
      </Link>
    );
  }
  ...
  return (
    ...
    >
      <span className={cn("flex items-center", s.wrap)}>
        ...
        />
        <span
          className={cn("text-gradient-brand font-bold leading-none tracking-tight", s.title)}
